...
knitr::opts_knit$set(root.dir = 
                "/Users/Emmanuel/code/mirna-breast-cancer/")
...
We clean the sample info file (Run only planning on using DESeq2)
...
# The sample_info file has not the same info as the miRNA dataframe. We create a new file
meta = data.frame(cases=colnames(counts_df))
rownames(meta) <- colnames(counts_df)

# Obtain the "case' vs 'control' value for each sample
meta$condition <- apply(meta, 1,
                            function(x)  sample_info[sample_info$sample==x[1],]$condition[1])

# Delete the now useless dataframe
#rm(sample_info)

# Safety check (TRUE is expected)
table(rownames(meta) == meta$cases)

# Safety check ("Control" and "Case" are expected)
