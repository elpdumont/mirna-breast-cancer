...
testIndices <- sample(sampleIndices, trunc(length(sampleIndices)/5))
...
controlsTrain = normTrain[grepl("Control", colnames(normTrain))]
casesTrain = normTrain[grepl("Case", colnames(normTrain))]

# Create a column with the row names
normTrain$mirna = rownames(normTrain)

# suppressWarnings(

# Compute the  p-value of a t-test for each miRNA
normTrain$pValue <- apply(normTrain, 1,
...
normTrain$controlMean <- apply(normTrain, 1,
                              function(x) round(mean( 
                                as.numeric( controlsTrain[x['mirna'], ] ), 
                                na.rm = FALSE ),3))

normTrain$caseMean <- apply(normTrain, 1,
                              function(x) round(mean( 
                                as.numeric(casesTrain[x['mirna'], ]), 
                                na.rm = FALSE ), 3))

normTrain$effectSize <- round(normTrain$caseMean / normTrain$controlMean, 3)
                              
# Correct for multiple testing using Benjamini & Hochberg 
# criteria (commented because not used)
normTrain$adjPValue <- p.adjust(normTrain$pValue, method = "BH")

# Delete intermediary files
rm(controlsTrain, casesTrain)

# Display statistics on the effect size
summary(normTrain$effectSize)

# Display statistics on the p-value
summary(normTrain$pValue)

#----------------------------------------------------
# Remove the miRNA where the effect size is NA
normTrain = normTrain[!is.na(normTrain$effectSize), ]
...
normTrainFilt <- normTrain[normTrain$effectSize > minEffectSize |
                           normTrain$effectSize < 1/minEffectSize, ]

# Filter based on p-value
normTrainFilt = normTrainFilt[normTrainFilt$pValue < maxPValue, ]

# Create an array of the selected miRNAs
mirna <- rownames(normTrainFilt)
print(mirna)

# Create the equivalent dataset for testing
normTestFilt <- normTest[mirna, ]
...
For each miRNA, we normalize by usign the formula $(x-\mu)/\sigma$ where $\mu$ is the mean of all samples for a given miRNA and where $\sigma$ is the standard deviation.
