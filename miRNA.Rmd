...
x <- model.matrix(condition~., train_scaled_df)[,-1]
# Convert the outcome (class) to a numerical variable
y <- ifelse(train_scaled_df$condition == "Case", 1, 0)
...
# Make predictions on the testing data (DO NOT DO! - that was just a test)
# lasso_model <- glmnet(x, y, alpha = 1, family = "binomial",
#                       lambda = cv_lasso$lambda.min)
# x.test <- model.matrix(condition ~., train.data)[,-1]
# probabilities <- lasso_model %>% predict(newx = x.test, type="response")
# predicted.classes <- ifelse(probabilities > 0.5, "Case", "Control")
# observed.classes <- train.data$condition
# model_accuracy = mean(predicted.classes == observed.classes)
# cat("The accuracy is", model_accuracy, "\n")
# train.data$condition_binary = ifelse(train.data$condition == "Control", 0, 1)
# pred <- prediction(probabilities,train.data$condition_binary)
...
# Model accuracy
observed.classes <- test_scaled_df$condition
model_accuracy = mean(predicted.classes == observed.classes)
cat("The accuracy is", model_accuracy, "\n")

# ROC
test_scaled_df$condition_binary = ifelse(test_scaled_df$condition == "Control", 0, 1)
pred <- prediction(probabilities,test_scaled_df$condition_binary)
perf <- performance(pred,"tpr","fpr")
par(pty="s")
...
# Create a column 
scaled.svm <- scaled_df
rows <- sample(nrow(scaled.svm))
scaled.svm = scaled.svm[rows, ]

scaled.svm$condition = as.factor(scaled.svm$condition)

# SVM

index <- 1:nrow(scaled.svm)
testindex <- sample(index, trunc(length(index)/5))
testset <- scaled.svm[testindex,  ]
trainset <- scaled.svm[-testindex, ]

svm.model <- svm(condition ~ ., data = trainset, kernel = "linear", cost = 100, gamma = 1)
svm.pred <- predict(svm.model, testset[ , !names(testset) %in% c('condition')] )

# Confusion matrix
table(pred = svm.pred, true = testset[, c('condition')])

# Model accuracy
observed.classes <- testset$condition
model_accuracy = mean(svm.pred == observed.classes)
cat("The accuracy is", model_accuracy, "\n")
# 
# # ROC curve
# perf <- performance(pred = svm.pred,"tpr","fpr")
# par(pty="s")
# plot(perf,  main = "ROC curve")
# # plot the no-prediction line
# lines(c(0,1),c(0,1),col = "gray", lty = 4 )
# auc_ROCR <- performance(svm.pred, measure = "auc")
#   auc_ROCR <- auc_ROCR@y.values[[1]]
# 
# cat("The AUC is", auc_ROCR)  
